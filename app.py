...
save_clicked = st.button("💾 Save Entry")  # single save button

new_row = None  # initialize

# ---------------- SALE / PURCHASE ----------------
if entry_category == "Sale / Purchase":
    col1,col2,col3 = st.columns(3)
    with col1:
        item = st.selectbox("Item",["गेहू खरीदा","आटा बेचा","चावल खरीदा","चावल बेचा","सरसों खरीदी","सरसों तेल बेचा"])
    with col2:
    ...
    if qty_f is not None and rate_f is not None:
        ...
        color="red" if "खरीद" in item else "green"
        text="Expense" if "खरीद" in item else "Income"
        st.markdown(f"### {text}: ₹ {preview}", unsafe_allow_html=True)
    if save_clicked:
        if qty_f is None or rate_f is None:
            st.error("❌ Quantity aur Rate sahi likho")
        else:
            total = qty_f*rate_f
            total = -abs(total) if "खरीद" in item else abs(total)
            ttype = "Expense" if "खरीद" in item else "Income"
            new_row = {
...
# ---------------- ELECTRICITY BILL ----------------
    ...
    bill_month = st.selectbox("Bill Month", ["January","February","March","April","May","June","July","August","September","October","November","December"])
...
if not df.empty:
    ...
    st.download_button("⬇ Download Report", filtered.to_csv(index=False), "report.csv", "text/csv")
